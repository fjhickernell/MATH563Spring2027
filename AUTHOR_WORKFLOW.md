...
Use `quarto preview` for the website. The standard notebook runtime is the `qmcpy` environment/kernel at `/opt/miniconda3/envs/qmcpy`; the carried-forward slides execute Python code. In the existing `qmcpy` environment, install `requirements.txt` and the pinned library as needed before rendering; do not replace the shared runtime with a generic kernel. When notebooks are added, document their packages, validate full local execution and timing output, and record instructor review.
...
## Carried-Forward Slides

Five MATH 563 Spring 2026 deck sources retain their instructional bodies and consume the newer pinned library. They are marked as historical material awaiting revision; inherited dates, survey results, and presentation comments are not Spring 2027 requirements. Review those details, mathematics, and companion links before adopting the decks for teaching.
